document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Paint Visualizer Logic ---
    const roomBtns = document.querySelectorAll('.room-btn');
    const baseImage = document.getElementById('baseImage');
    const wallArea = document.getElementById('wallArea'); // Targeted SVG Path
    const opacitySlider = document.getElementById('opacitySlider');
    const resetVisualizer = document.getElementById('resetVisualizer');
    ...
    // Room specific SVG paths
    const roomPaths = {
        living: "M 350,0 L 1000,0 L 1010,750 L 350,750 Z",
        bedroom: "M 0,0 L 1000,0 L 1005,650 L 0,650 Z",
        exterior: "M 150,0 L 850,0 L 880,1000 L 120,1000 Z"
    };
    ...
    roomBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            baseImage.src = `visualizer_${room}.png`;

            // Update SVG wall area path
            if (roomPaths[room]) {
                wallArea.setAttribute('d', roomPaths[room]);
            }
        });
    });

    // Apply Color to Wall
    window.applyColorToVisualizer = (hex) => {
        selectedColor = hex;
        wallArea.style.fill = hex;
        wallArea.style.fillOpacity = opacitySlider.value;
    ...
    document.getElementById('applyVisualizer').addEventListener('click', () => {
        if (activeColour) {
            applyColorToVisualizer(activeColour.hex);
            modal.style.display = 'none';
            document.body.style.overflow = 'auto';
        }
    });

    // Opacity / Intensity Slider (Only affects wallArea)
    opacitySlider.addEventListener('input', (e) => {
        if (selectedColor) {
            wallArea.style.fillOpacity = e.target.value;
        }
    });

    // Reset Visualizer
    resetVisualizer.addEventListener('click', () => {
        selectedColor = null;
        wallArea.style.fill = 'transparent';
    ...
    toggleComparison.addEventListener('click', () => {
        ...
        if (isComparing) {
            toggleComparison.innerText = 'Compare: ON';
            wallArea.style.display = "none";
        } else {
            toggleComparison.innerText = 'Compare: OFF';
            wallArea.style.display = "block";
        }
    });

    // Download Design (Using html2canvas for accurate capture)
    downloadDesign.addEventListener('click', () => {
        if (!selectedColor) {
            alert("Please apply a color first to download your design!");
            return;
        }

        const btnText = downloadDesign.innerHTML;
        downloadDesign.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Preparing...';

        html2canvas(visualizerContainer, {
            useCORS: true,
            allowTaint: true,
            scale: 2
        }).then(canvas => {
            ...
            link.click();
            downloadDesign.innerHTML = btnText;
        }).catch(err => {
            console.error("Download failed:", err);
            alert("Design capture failed. Please try again.");
            downloadDesign.innerHTML = btnText;
        });
    });
